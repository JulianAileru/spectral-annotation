...
_WINE_PREFIX_SCRIPT = '/usr/local/bin/run_with_wine_prefix.sh'
_WINE_EXE = 'wine64'
_MSPEPSEARCH_EXE = '/opt/mspepsearch/MSPepSearch64.exe'


def _to_wine_path(linux_path: str) -> str:
...
class MSPepSearch:
    ...
    def _build_cmd(self, wine_input: str, wine_libs: list[str], wine_output: str, lib_in_mem: bool = True) -> list[str]:
        cmd = [_WINE_PREFIX_SCRIPT, _WINE_EXE, _MSPEPSEARCH_EXE, self._build_positional_flags()]
        cmd += ["/INP", wine_input]
        for lib in wine_libs:
            cmd += ["/LIB", lib]
        if lib_in_mem:
            cmd += ["/LibInMem"]
        if self.is_hires:
        ...
        cmd += ["/OUTTAB", wine_output]
        cmd += ["/HITS", str(self.max_hits)]
        cmd += ["/MinMF", str(self.min_match_factor)]
        cmd += ["/OutPrecursorType", "/OutMW", "/OutChemForm", "/OutIK", "/OutDeltaMW", "/OutNumMP"]
    ...
    def _build_container_cmd(self, output_path: str) -> list[str]:
        ...
        for i, lib in enumerate(self.lib_paths):
            lib_binds.append(f"{Path(lib).resolve().as_posix()}:/work/lib{i}")
            container_lib_paths.append(f"/work/lib{i}")

        wine_input = _to_wine_path("/work/input.msp")
        wine_output = _to_wine_path(container_output)
        wine_libs = [_to_wine_path(lib) for lib in container_lib_paths]

        mspepsearch_cmd = self._build_cmd(wine_input, wine_libs, wine_output)
